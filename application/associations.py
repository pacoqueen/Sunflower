...
class AssociationManager:
    ...
    def exec_launcher(self, path, uris = None):
        """Launch x-desktop files instead of let it open with gedit or
        whatever text editor is associated with.
        Uses an external tool until all sunflower will be fully ported
        to Gtk3"""
        ext_tool = os.path.join(os.path.dirname(__file__),
                                "tools", "run-desktop.py")
        subprocess.Popen(
                    (ext_tool, path, '&'),
    ...
    def execute_file(self, path, provider=None):
        ...
        if gio.content_type_can_be_executable(mime_type) and should_execute:
            # Check for desktop file only if is set as executable
            if mime_type == "application/x-desktop":
                self.exec_launcher(path)
